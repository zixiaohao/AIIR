...
class RateLimiter:
    ...
    def is_allowed(self, ip, max_requests, time_window_hours):
        ...
        if not current_config.get('rate_limit', {}).get('enabled', False):
            self.total_requests[ip] += 1
            return True
        
        # 使用最新配置
        max_requests = current_config['rate_limit'].get('max_requests_per_ip', max_requests)
        time_window_hours = current_config['rate_limit'].get('time_window_hours', time_window_hours)
