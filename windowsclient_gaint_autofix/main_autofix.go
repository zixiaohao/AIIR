...
func sendDataToServer(ticketID, hostname string) {
	...
	// 发送请求 - 使用 /analyze_with_actions 接口获取分析报告和修复动作
	fmt.Printf("[发送请求] %s\n", serverURL)
	fmt.Println("[AI模式] 带自动修复动作的分析")
	resp, err := http.Post(serverURL+"/analyze_with_actions", "application/json", bytes.NewBuffer(jsonBody))
	if err != nil {
