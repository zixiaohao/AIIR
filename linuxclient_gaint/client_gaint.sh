...
# ================= 使用说明 =================
# 使用方法:
#   ./client_gaint.sh -s http://SERVER_IP:8000
#   ./client_gaint.sh --server http://SERVER_IP:8000
#   ./client_gaint.sh -h  # 查看帮助
#   ./client_gaint.sh     # 交互式输入服务器地址
# ===========================================

# 显示帮助信息
show_help() {
    echo "Linux 安全应急响应检测工具 v3.1 (gaint版本)"
    echo ""
    echo "用法:"
    echo "  ./client_gaint.sh -s <Server地址>"
    echo "  ./client_gaint.sh --server <Server地址>"
    echo "  ./client_gaint.sh -h  # 显示此帮助信息"
    echo ""
    echo "参数:"
    echo "  -s, --server    指定Server地址 (格式: http://IP:端口)"
    echo "  -h, --help      显示此帮助信息"
    echo ""
    echo "示例:"
    echo "  ./client_gaint.sh -s http://192.168.1.100:8000"
    echo "  ./client_gaint.sh --server http://10.0.0.50:8000"
    echo ""
    echo "注意:"
    echo "  - 脚本需要root权限运行"
    echo "  - 如果不指定Server地址，脚本将询问用户输入"
    echo "  - 离线模式下脚本仍可执行安全检查但无法发送分析请求"
    echo "  - gaint版本采用一次性发送模式，适合大模型上下文窗口较大的场景"
    exit 0
}
...
# 如果没有指定Server地址，提示用户输入
if [ -z "$SERVER_URL" ]; then
    read -p "请输入Server地址 (格式: http://IP:端口): " SERVER_URL
    if [ -z "$SERVER_URL" ]; then
...
# 发送完整日志到Server（使用Server默认模型）
response=$(curl -s -X POST "${SERVER_URL}/analyze_full" \
